...
class Database:
    def __init__(self,db): #self = this
        self.conn = sqlite3.connect(db)
        self.cur = self.conn.cursor() 
        self.cur.execute("CREATE TABLE IF NOT EXISTS parts (id INTEGER PRIMARY KEY,part text,customer text, retailer text, price text)")# to exec queries
        self.conn.commit()
...
#b.insert("Asus Mobo", "Mike Henry", "Microcenter", "360")
